...
export default function AnalyticsView({ language, translations }) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex flex-col items-center justify-center p-6 min-h-[40vh] text-center">
        <div className="w-12 h-12 bg-health-primary/10 dark:bg-health-primary/20 rounded-2xl flex items-center justify-center text-xl mb-4 shadow-inner">
          🔐
        </div>
        <h3 className="text-base font-bold text-slate-800 dark:text-slate-200 mb-2 font-serif">
        ...
        <form onSubmit={handleVerifyPasscode} className="w-full max-w-xs space-y-3">
          <input
            ...
            className="w-full px-3.5 py-2 text-center text-sm bg-white dark:bg-slate-800 text-gray-900 dark:text-gray-100 border border-slate-350 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-health-primary focus:border-transparent transition"
          />
          {passcodeError && (
            <p className="text-[10px] text-health-emergency font-semibold">{passcodeError}</p>
          )}
          <button
            type="submit"
            className="w-full py-2 text-xs font-bold text-white bg-health-primary hover:bg-health-primary/90 rounded-xl transition shadow-sm font-serif"
          >
            {language === 'hi' ? 'सत्यापित करें' : language === 'gu' ? 'ચકાસો' : 'Verify Access'}
          </button>
        </form>
        <p className="text-[10px] text-gray-405 dark:text-gray-500 mt-4">
          Demo passcode: <code className="bg-gray-105 dark:bg-slate-800 px-1.5 py-0.5 rounded font-mono">{DEMO_PASSCODE}</code>
        </p>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center min-h-[40vh]">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-health-primary mb-3"></div>
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-center min-h-[40vh]">
        ...
        <h4 className="text-sm font-bold text-health-emergency font-serif">Failed to load analytics</h4>
  ...
  return (
    <div className="space-y-6">
      {/* Title */}
      <div>
        <h2 className="text-base font-extrabold text-slate-800 dark:text-slate-100 flex items-center gap-2 font-serif">
          <span>📊</span>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          {language === 'hi' 
            ...
            : language === 'gu' 
            ? 'હેલ્થ સાથી સિસ્ટમમાં સક્રિય અહેવાલોનું લાઈવ સુરક્ષित દૃશ્ય.' 
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-2xl border border-slate-200 dark:border-slate-800/50">
          <span className="text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500 font-bold block mb-1">
            Total Conversations
          </span>
          <span className="text-base font-extrabold text-health-primary dark:text-health-secondary font-mono">
        ...
        <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-2xl border border-slate-200 dark:border-slate-800/50">
          ...
          <span className="text-base font-extrabold text-health-success font-serif">
            {mostCommonSev}
          </span>
        </div>
      </div>
      ...
      <div className="bg-slate-50/50 dark:bg-slate-800/20 p-4 rounded-2xl border border-slate-200 dark:border-slate-800/50">
        <h3 className="text-xs font-bold text-slate-700 dark:text-slate-300 mb-3 font-serif">
          Severity Case Distribution
        </h3>
        <div className="space-y-3.5">
          {severityDist.map(item => {
            let barColor = 'bg-health-success';
            if (item.severity.toLowerCase() === 'doctor') barColor = 'bg-health-caution';
            if (item.severity.toLowerCase() === 'emergency') barColor = 'bg-health-emergency';
            ...
            return (
              <div key={item.severity}>
                <div className="flex justify-between items-center text-xs text-slate-650 dark:text-slate-350 mb-1">
                  ...
                  <span className="font-bold font-mono">{item.count} ({item.percentage}%)</span>
                </div>
            ...
            {data.symptom_counts.map((symptom) => {
              ...
              return (
                <div key={symptom.name} className="flex items-center gap-3">
                  <span className="text-xs font-bold text-slate-500 dark:text-slate-400 w-24 truncate font-serif" title={symptom.name}>
                    {symptom.name}
                  </span>
                  <div className="flex-1 bg-slate-200 dark:bg-slate-700 h-3 rounded-md overflow-hidden relative">
                    <div className="h-full bg-health-primary/80 dark:bg-health-secondary/80 rounded-md transition-all duration-500" style={{ width: `${percent}%` }}></div>
                  </div>
      ...
      {/* Privacy Notice */}
      <div className="bg-slate-100/40 dark:bg-slate-800/20 border border-slate-200 dark:border-slate-800/60 rounded-xl p-3.5 flex items-start gap-2.5 shadow-sm">
        <span className="text-sm">🛡️</span>
        <p className="text-[10px] md:text-xs text-slate-650 dark:text-slate-400 font-medium leading-relaxed">
